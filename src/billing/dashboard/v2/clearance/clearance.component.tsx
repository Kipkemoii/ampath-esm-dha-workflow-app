...
const ClearanceTable: React.FC<{
  ...
  if (rows.length === 0) {
    return <p className={styles.empty}>{status === 'AWAITING_PAYMENT' ? 'No patients awaiting clearance.' : 'No cleared patients yet.'}</p>;
  }
...
const Clearance: React.FC<{ onChange?: () => void }> = ({ onChange }) => {
