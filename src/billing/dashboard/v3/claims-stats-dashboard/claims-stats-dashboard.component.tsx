...
const ClaimsStatsDashboard: React.FC<ClaimsStatsDashboardProps> = ({locationUuid,reportDate})=>{
      ...
      const submissionReadyClaims:ClaimVisit[] = useMemo(()=>getClaimByType(ClaimProviderStatus.SubmissionReady),[claimVisits]);
      const submittedClaims:ClaimVisit[] = useMemo(()=>getClaimByType(ClaimProviderStatus.Submitted),[claimVisits]);
      const rejectedClaims:ClaimVisit[] = useMemo(()=>getClaimByType(ClaimProviderStatus.Submitted, ClaimPayerStatus.Rejected),[claimVisits]);
      const approvedClaims:ClaimVisit[] = useMemo(()=>getClaimByType(ClaimProviderStatus.Submitted, ClaimPayerStatus.Approved),[claimVisits]);
      const paidClaims:ClaimVisit[] = useMemo(()=>getClaimByType(ClaimProviderStatus.Submitted, ClaimPayerStatus.Paid),[claimVisits]);
      useEffect(() => {
...
function getClaimByType(providerStatus: string, payerStatus?: string){
  ...
  <div className={styles.claimStatsLayout}>
       ...
       <div className={styles.summaryRow}>
           ...
           <ClaimStat title={ClaimProviderStatus.Draft} count={draftClaims.length} onStatClick={handleIndicatorSelection}/>
           <ClaimStat title={ClaimProviderStatus.Closed} count={closedClaims.length} onStatClick={handleIndicatorSelection}/>
           <ClaimStat title={ClaimProviderStatus.SubmissionReady} count={submissionReadyClaims.length} onStatClick={handleIndicatorSelection}/>
           <ClaimStat title={ClaimProviderStatus.Submitted} count={submittedClaims.length} onStatClick={handleIndicatorSelection}/>
           <ClaimStat title={ClaimPayerStatus.Approved} count={approvedClaims.length} onStatClick={handleIndicatorSelection}/>
           <ClaimStat title={ClaimPayerStatus.Rejected} count={rejectedClaims.length} onStatClick={handleIndicatorSelection}/>
